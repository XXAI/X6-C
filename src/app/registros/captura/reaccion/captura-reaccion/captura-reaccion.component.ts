...
@Component({
  selector: 'app-captura-reaccion',
  templateUrl: './captura-reaccion.component.html',
  styleUrls: ['./captura-reaccion.component.css']
})
export class CapturaReaccionComponent implements OnInit {
  ...
  constructor(private title: Title,
    ...
    ngOnInit() {
      this.title.setTitle("Registro Reacciones Adversas");
      ...
      var busquedaSubject = this.terminosBusqueda
        ...
        .switchMap((term:string)  =>  { 
          ...
          this.cargando = true;
          if(bandera)
            return term  ? this.reaccionService.buscar_detalle(term, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda, this.busqueda.id_jurisdiccion) : Observable.of<any>({data:[]});
          else
            return this.reaccionService.buscar_detalle(term, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda, this.busqueda.id_jurisdiccion);
        }
    ...
    listarBusqueda(term:string ,pagina:number): void {
      ...
      this.cargando = true;
      this.reaccionService.buscar_detalle(term, pagina, this.resultadosPorPaginaBusqueda, this.busqueda.id_jurisdiccion).subscribe(
          resultado => {
            this.cargando = false;
    ...
    filtro(id:number, value:string):void
       {
          ...
          {
            this.busquedaActivada = true;
            this.reaccionService.buscar(this.ultimoTerminoBuscado, this.busqueda, 1, this.resultadosPorPaginaBusqueda).subscribe(
                resultado => {
                  this.cargando = false;
    ...
    cargar_catalogos():void{
      this.reaccionService.carga_catalogos().subscribe(
          resultado => {
    ...
    listar(pagina:number): void {
      ...
      this.cargando = true;
      this.reaccionService.lista(pagina,this.resultadosPorPagina).subscribe(
          resultado => {
            
            this.cargando = false;
            this.verificacion = resultado.data as Reaccion[];
