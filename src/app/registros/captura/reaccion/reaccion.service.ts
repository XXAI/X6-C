...
export class ReaccionService {
  ...
  buscar_detalle(term: string, pagina:number = 1, resultados_por_pagina:number =20, id_jurisdiccion: any): Observable<any>{
    return this.jwtRequest.get(ReaccionService.URL,null,{q: term, page: pagina, per_page: resultados_por_pagina, jurisdiccion: id_jurisdiccion}).map( (response: Response) => response.json().data);
  }
