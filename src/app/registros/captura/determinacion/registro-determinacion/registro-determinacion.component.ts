...
@Component({
  selector: 'app-registro-determinacion',
  templateUrl: './registro-determinacion.component.html',
  styleUrls: ['./registro-determinacion.component.css']
})
export class RegistroDeterminacionComponent implements OnInit {
  ...
  constructor(private title: Title,
              ...
              private registrodeterminacionService: RegistroDeterminacionService,
  ...
  ngOnInit() {
    ...
    this.title.setTitle("Registros Determinacion");
  ...
  listar(pagina:number): void {
    ...
    this.cargando = true;
    this.registrodeterminacionService.lista(pagina,this.resultadosPorPagina, this.local).subscribe(
        resultado => {
          
          this.cargando = false;
          this.registros = resultado.data as Determinacion[];
    ...
    listarBusqueda(term:string ,pagina:number): void {
      ...
      this.cargando = true;
      this.registrodeterminacionService.buscar_detalle(term, pagina, this.resultadosPorPaginaBusqueda).subscribe(
          resultado => {
            this.cargando = false;
    ...
    cargar_catalogos(obj:any):void{
      this.registrodeterminacionService.carga_catalogos(obj).subscribe(
          resultado => {
    ...
    agregar() {
      ...
      {  
        ...
        formData.append("especificaciones", this.registrosModule.get("especificaciones").value);
        ...
        var self = this;
        if(this.id_editar == 0)  
          xhr.open("POST", environment.API_URL+"/registro-determinacion", true);
        else  
          xhr.open("POST", environment.API_URL+"/registro-determinacion/"+this.id_editar, true);
  ...
  descargar(obj:any) {
      ...
      var self = this;
      var download = window.open(`${environment.API_URL}/descargar-determinacion/${obj.id}?${query}`);  
  }

  descargar_archivo() {
    var query = "token="+localStorage.getItem('token');
    var self = this;
    var download = window.open(`${environment.API_URL}/descargar-determinacion/${this.id_editar}?${query}`);  
}
error_descargar(obj)
{
  ...
  error_envio(obj)
  {
    ...
    eliminar(id:string):void
    {
        ...
        {
            this.cargando = true;
            this.registrodeterminacionService.elimina(id).subscribe(
            resultado => {
              this.cargando = false;
              this.listar(1);
