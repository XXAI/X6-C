...
@Component({
  ...
  templateUrl: './captura-capacitacion.component.html',
  styleUrls: ['./captura-capacitacion.component.css']
})
export class CapturaCapacitacionComponent implements OnInit {
  ...
  resultadosBusqueda: Capacitacion[] = [];
  ...
  constructor(private title: Title,
    ...
    ngOnInit() {
      ...
      var busquedaSubject = this.terminosBusqueda
        ...
        .switchMap((term:string)  =>  { 
          ...
          this.cargando = true;
          if(bandera)
            return term  ? this.capacitacionService.buscar_detalle(term, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda, this.busqueda.id_jurisdiccion) : Observable.of<any>({data:[]});
          else
            return this.capacitacionService.buscar_detalle(term, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda, this.busqueda.id_jurisdiccion);
        }
        ...
        busquedaSubject.subscribe(
          resultado => {
            this.cargando = false;
            this.resultadosBusqueda = resultado.data as Capacitacion[];
    ...
    listarBusqueda(term:string ,pagina:number): void {
      ...
      this.cargando = true;
      this.capacitacionService.buscar_detalle(term, pagina, this.resultadosPorPaginaBusqueda, this.busqueda.id_jurisdiccion).subscribe(
          resultado => {
            this.cargando = false;
    ...
    filtro(id:number, value:string):void
       {
          ...
          {
            this.busquedaActivada = true;
            this.capacitacionService.buscar(this.ultimoTerminoBuscado, this.busqueda, 1, this.resultadosPorPaginaBusqueda).subscribe(
                resultado => {
                  this.cargando = false;
    ...
    cargar_catalogos():void{
      this.capacitacionService.carga_catalogos().subscribe(
          resultado => {
    ...
    listar(pagina:number): void {
      ...
      this.cargando = true;
      this.capacitacionService.lista(pagina,this.resultadosPorPagina).subscribe(
          resultado => {
            
            this.cargando = false;
            this.verificacion = resultado.data as Capacitacion[];
