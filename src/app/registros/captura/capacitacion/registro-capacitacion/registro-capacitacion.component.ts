...
export class RegistroCapacitacionComponent implements OnInit {
  ...
  ngOnInit() {
    this.registrosModule = this.fb.group({
      ...
      archivo: ['', []],
      material_difusion: ['', [Validators.required]],
      no_verificacion: ['', [Validators.required]],
      institucion: ['', [Validators.required]],
      id_verificacion: ['', [Validators.required]]
      });

    this.title.setTitle("Registros Capacitación");
  ...
  listar(pagina:number): void {
    ...
    this.cargando = true;
    this.registrocapacitacionService.lista(pagina,this.resultadosPorPagina, this.local).subscribe(
        resultado => {
          
          this.cargando = false;
          this.registros = resultado.data as Capacitacion[];
    ...
    listarBusqueda(term:string ,pagina:number): void {
      ...
      this.cargando = true;
      this.registrocapacitacionService.buscar_detalle(term, pagina, this.resultadosPorPaginaBusqueda).subscribe(
          resultado => {
            this.cargando = false;
    ...
    cargar_catalogos(obj:any):void{
      this.registrocapacitacionService.carga_catalogos(obj).subscribe(
          resultado => {
    ...
    agregar() {
      ...
      {  
        ...
        formData.append("material_difusion", this.registrosModule.get("material_difusion").value);
        ...
        var self = this;
        if(this.id_editar == 0)  
          xhr.open("POST", environment.API_URL+"/registro-capacitacion", true);
        else  
          xhr.open("POST", environment.API_URL+"/registro-capacitacion/"+this.id_editar, true);
...
error_descargar(obj)
{
  ...
  error_envio(obj)
  {
    ...
    eliminar(id:string):void
    {
        ...
        {
            this.cargando = true;
            this.registrocapacitacionService.elimina(id).subscribe(
            resultado => {
              this.cargando = false;
              this.listar(1);
    ...
    editar(obj:any)
    {
      this.reset_form();
      this.showregistro = true;  
      this.registrosModule.patchValue({material_difusion:obj.material_difusion, id_verificacion:obj.verificacion.id, no_verificacion:obj.verificacion.folio, institucion: obj.verificacion.establecimiento});
