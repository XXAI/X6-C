...
export class RegistroMuestraComponent implements OnInit {
  ...
  constructor(private title: Title,
              ...
              private registromuestraService: RegistroMuestraService,
  ...
  ngOnInit() {
    ...
    this.title.setTitle("Registros Muestra");
  ...
  listar(pagina:number): void {
    ...
    this.cargando = true;
    this.registromuestraService.lista(pagina,this.resultadosPorPagina, this.local).subscribe(
        resultado => {
          
          this.cargando = false;
          this.registros = resultado.data as Muestra[];
    ...
    listarBusqueda(term:string ,pagina:number): void {
      ...
      this.cargando = true;
      this.registromuestraService.buscar_detalle(term, pagina, this.resultadosPorPaginaBusqueda).subscribe(
          resultado => {
            this.cargando = false;
    ...
    agregar() {
      ...
      {  
        ...
        var self = this;
        if(this.id_editar == 0)  
          xhr.open("POST", environment.API_URL+"/registro-muestra", true);
        else  
          xhr.open("POST", environment.API_URL+"/registro-muestra/"+this.id_editar, true);
  ...
  descargar(obj:any) {
      ...
      var self = this;
      var download = window.open(`${environment.API_URL}/descargar-muestra/${obj.id}?${query}`);  
  }

  descargar_archivo() {
    var query = "token="+localStorage.getItem('token');
    var self = this;
    var download = window.open(`${environment.API_URL}/descargar-muestra/${this.id_editar}?${query}`);  
}
error_descargar(obj)
{
  ...
  error_envio(obj)
  {
    ...
    eliminar(id:string):void
    {
        ...
        {
            this.cargando = true;
            this.registromuestraService.elimina(id).subscribe(
            resultado => {
              this.cargando = false;
              this.listar(1);
