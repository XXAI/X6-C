...
@Component({
  selector: 'app-captura-muestra',
  templateUrl: './captura-muestra.component.html',
  styleUrls: ['./captura-muestra.component.css']
})
export class CapturaMuestraComponent implements OnInit {
  ...
  constructor(private title: Title,
    ...
    ngOnInit() {
      ...
      var busquedaSubject = this.terminosBusqueda
        ...
        .switchMap((term:string)  =>  { 
          ...
          this.cargando = true;
          if(bandera)
            return term  ? this.muestraService.buscar_detalle(term, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda, this.busqueda.id_jurisdiccion) : Observable.of<any>({data:[]});
          else
            return this.muestraService.buscar_detalle(term, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda, this.busqueda.id_jurisdiccion);
        }
        ...
        busquedaSubject.subscribe(
          resultado => {
            this.cargando = false;
            this.resultadosBusqueda = resultado.data as Muestra[];
    ...
    listarBusqueda(term:string ,pagina:number): void {
      ...
      this.cargando = true;
      this.muestraService.buscar_detalle(term, pagina, this.resultadosPorPaginaBusqueda, this.busqueda.id_jurisdiccion).subscribe(
          resultado => {
            this.cargando = false;
    ...
    filtro(id:number, value:string):void
       {
          ...
          {
            this.busquedaActivada = true;
            this.muestraService.buscar(this.ultimoTerminoBuscado, this.busqueda, 1, this.resultadosPorPaginaBusqueda).subscribe(
                resultado => {
                  this.cargando = false;
    ...
    cargar_catalogos():void{
      this.muestraService.carga_catalogos().subscribe(
          resultado => {
    ...
    listar(pagina:number): void {
      ...
      this.cargando = true;
      this.muestraService.lista(pagina,this.resultadosPorPagina).subscribe(
          resultado => {
            
            this.cargando = false;
            this.verificacion = resultado.data as Muestra[];
