...
@Component({
  selector: 'app-captura-dictamen',
  templateUrl: './captura-dictamen.component.html',
  styleUrls: ['./captura-dictamen.component.css']
})
export class CapturaDictamenComponent implements OnInit {
  ...
  constructor(private title: Title,
    ...
    ngOnInit() {
      this.title.setTitle("Registro Dictamenes");
      ...
      var busquedaSubject = this.terminosBusqueda
        ...
        .switchMap((term:string)  =>  { 
          ...
          this.cargando = true;
          if(bandera)
            return term  ? this.dictamenService.buscar_detalle(term, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda, this.busqueda.id_jurisdiccion) : Observable.of<any>({data:[]});
          else
            return this.dictamenService.buscar_detalle(term, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda, this.busqueda.id_jurisdiccion);
        }
    ...
    listarBusqueda(term:string ,pagina:number): void {
      ...
      this.cargando = true;
      this.dictamenService.buscar_detalle(term, pagina, this.resultadosPorPaginaBusqueda, this.busqueda.id_jurisdiccion).subscribe(
          resultado => {
            this.cargando = false;
    ...
    filtro(id:number, value:string):void
       {
          ...
          {
            this.busquedaActivada = true;
            this.dictamenService.buscar(this.ultimoTerminoBuscado, this.busqueda, 1, this.resultadosPorPaginaBusqueda).subscribe(
                resultado => {
                  this.cargando = false;
    ...
    cargar_catalogos():void{
      this.dictamenService.carga_catalogos().subscribe(
          resultado => {
    ...
    listar(pagina:number): void {
      ...
      this.cargando = true;
      this.dictamenService.lista(pagina,this.resultadosPorPagina).subscribe(
          resultado => {
            
            this.cargando = false;
            this.verificacion = resultado.data as Dictamen[];
