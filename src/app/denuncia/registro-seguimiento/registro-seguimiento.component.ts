...
export class RegistroSeguimientoComponent implements OnInit {
  ...
  obj_denuncia:any = {};
  modalSeguimiento:boolean = false;
  ...
  ngOnInit() { 
    this.seguimientoModule = this.fb.group({
      ...
      idEstatus: ['2', [Validators.required]]
    });
  ...
  cargar_denuncia(id:string):void
  {
    ...
    this.seguimientodenunciaService.seguimiento_denuncia(obj).subscribe(
    respuesta => {
  ...
  {
    this.cargando = true;
    this.seguimientodenunciaService.registrar_seguimiento(this.folio, this.seguimientoModule.value).subscribe(
    respuesta => {
