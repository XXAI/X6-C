...
export class ListaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    var busquedaSubject = this.terminosBusqueda
        ...
        if(this.busqueda.buscarText.length == 0 && this.busqueda.id_jurisdiccion == 0)
        {
	      ...
	      this.cargando = true;
	      return term  ? this.verificacionService.buscar_detalle(term, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda) : Observable.of<any>({data:[]}) 
	    }
  ...
  listarBusqueda(term:string ,pagina:number): void {
    ...
    this.cargando = true;
    this.verificacionService.buscar_detalle(term, pagina, this.resultadosPorPaginaBusqueda).subscribe(
        resultado => {
          this.cargando = false;
