...
export class ListaRegistrosComponent implements OnInit {
  ...
  listar(pagina:number): void {
    ...
    this.cargando = true;
    this.registrosService.lista(pagina,this.resultadosPorPagina, this.local).subscribe(
        resultado => {
          
          this.cargando = false;
          this.registros = resultado.data as Registro[];
    ...
    listarBusqueda(term:string ,pagina:number): void {
      ...
      this.cargando = true;
      this.registrosService.buscar_detalle(term, pagina, this.resultadosPorPaginaBusqueda).subscribe(
          resultado => {
            this.cargando = false;
  
            this.resultadosBusqueda = resultado.data as Registro[];
    ...
    cargar_catalogos():void{
      this.registrosService.carga_catalogos().subscribe(
          resultado => {
    ...
    agregar() {
      if(this.tag)
      {
        ...
        formData.append("material_difusion", this.registrosModule.get("material").value);
        ...
        xhr.onreadystatechange = function () {
            if(xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
               ...
               self.registrosModule.patchValue({archivo:"", material:""});
               self.cargando = false; 
            }
  ...
  error_envio(obj)
  {
    ...
    eliminar(id:string):void
    {
        ...
        {
            this.cargando = true;
            this.registrosService.elimina(id).subscribe(
            resultado => {
              this.cargando = false;
              this.listar(1);
