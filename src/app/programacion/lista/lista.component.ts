...
export class ListaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    var busquedaSubject = this.terminosBusqueda
        ...
        if(this.busqueda.buscarText.length == 0 && this.busqueda.id_jurisdiccion == 0 && this.busqueda.id_tipo == 0)
        {
	      ...
	      this.cargando = true;
	      return term  ? this.metaService.buscar_detalle(term, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda,1) : Observable.of<any>({data:[]}) 
	    }
  ...
  listar(pagina:number): void {
    ...
    this.cargando = true;
    this.metaService.lista(pagina,this.resultadosPorPagina, 1).subscribe(
        resultado => {
    ...
    listarBusqueda(term:string ,pagina:number): void {
      ...
      this.cargando = true;
      this.metaService.buscar_detalle(term, pagina, this.resultadosPorPaginaBusqueda, 1).subscribe(
          resultado => {
            this.cargando = false;
    ...
    editarProgramacion(obj: any)
    {
      ...
      this.programacion_jurisdiccional.patchValue({id_jurisdiccion: obj.id_jurisdiccion, id_tema: obj.id_tema, id_tipo_programacion: obj.id_tipo_programacion});
