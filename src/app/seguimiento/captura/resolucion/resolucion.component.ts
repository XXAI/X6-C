...
export class ResolucionComponent implements OnInit {
  ...
  constructor( 
    ...
    ngOnInit() {
      ...
      this.registrosModule = this.fb.group({
        ...
        id_resolutor: ['', [Validators.required]],
        fecha_recepcion: ['', [Validators.required]],
        id_giro: ['', [Validators.required]],
        establecimiento: ['', [Validators.required]],
        id_procedencia: ['', [Validators.required]],
        no_citatorio: ['', [Validators.required]],
        fecha_notificacion_citatorio: ['', [Validators.required]],
        fecha_comparecencia: ['', [Validators.required]],
        no_resolucion: ['', [Validators.required]],
        fecha_emision: ['', [Validators.required]],
        id_sancion: ['', [Validators.required]],
        fecha_notificacion_resolucion: ['', [Validators.required]],
        id_recurso_inconformidad: ['', [Validators.required]],
        fecha_envio_juridico: ['', [Validators.required]],
        observaciones: ['', [Validators.required]],
        id_estatus: ['', [Validators.required]],
      });
  
      this.title.setTitle("Registro Resolución Administrativa");
      this.mensajeError = new Mensaje();
      this.mensajeExito = new Mensaje();
  
      this.listar(1);
      this.cargar_catalogos(3);
      ...
      var busquedaSubject = this.terminosBusqueda
        ...
        .switchMap((term:string)  =>  { 
          ...
          this.cargando = true;
          return busquedas_registradas ? this.principalService.buscar_resolucion(term, this.busqueda, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda) : Observable.of<any>({data:[]}) 
        }
  ...
  listar(pagina:number): void {
    ...
    this.cargando = true;
    this.principalService.lista_resolucion(pagina,this.resultadosPorPagina, 1).subscribe(
        resultado => {
          
          this.cargando = false;
          this.lista_resolucion = resultado.data as Seguimiento[];
  ...
  cargar_catalogos(tipo_acta:number): void {
    ...
    eliminar(id:string):void
    {
        ...
        {
            this.cargando = true;
            this.principalService.elimina_resolucion(id).subscribe(
            resultado => {
              this.cargando = false;
              this.listar(1);
    ...
    agregar_registro()
    {
      ...
      if(this.id_objeto== 0)
      {
        this.principalService.agregar_resolucion(this.registrosModule.value).subscribe(
          resultado => {
            this.listar(1);
            this.cargando = false;
            this.mensajeExito = new Mensaje(true);
        ...
        this.principalService.editar_resolucion(this.id_objeto, this.registrosModule.value).subscribe(
          resultado => {
            this.listar(1);
            this.cargando = false;
            this.mensajeExito = new Mensaje(true);
    ...
    cargar_registro(Objeto:any)
    {
      ...
      this.principalService.ver_resolucion(Objeto.id).subscribe(
          resultado => {
            this.showForm = true;
    ...
    listarBusqueda(term:string ,pagina:number): void {
      ...
      this.cargando = true;
      this.principalService.buscar_resolucion(term, pagina, this.resultadosPorPaginaBusqueda).subscribe(
          resultado => {
            this.cargando = false;
    ...
    filtro(id:number, value:string):void
    {
      ...
      {
        this.busquedaActivada = true;
        this.principalService.buscar_resolucion(this.ultimoTerminoBuscado, this.busqueda, 1, this.resultadosPorPaginaBusqueda).subscribe(
            resultado => {
              this.cargando = false;
