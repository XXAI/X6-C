...
export class NotificacionComponent implements OnInit {
  ...
  busqueda: any = {buscarText:'', areaoperativa: '0', procedimiento:'x'};
  lista_notificacion: Seguimiento[] = [];
  ...
  CatalogoProcedimiento: string[] = ['NOTIFICACIÓN', 'CUMPLIMIENTO A CITATORIO', 'INSTRUCTIVO'];
  ...
  constructor( 
    ...
    ngOnInit() {
      ...
      this.registrosModule = this.fb.group({
        ...
        id_notificador: ['', [Validators.required]],
        fecha_oficio_notificar: ['', [Validators.required]],
        id_tipo_notificacion: ['', [Validators.required]],
        establecimiento: ['', [Validators.required]],
        id_procedimiento_notificacion: ['', [Validators.required]],
        fecha_entrega_responsable: ['', [Validators.required]],
        fecha_notificacion: ['', [Validators.required]]
      });
  
      this.title.setTitle("Registro Notificación");
      ...
      var busquedaSubject = this.terminosBusqueda
        ...
        .switchMap((term:string)  =>  { 
          ...
          if(this.busqueda.buscarText.length == 0 && this.busqueda.areaoperativa == 0 && this.busqueda.procedimiento == 'x')
          {
          ...
          this.cargando = true;
          return busquedas_registradas ? this.principalService.buscar_notificacion(term, this.busqueda, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda) : Observable.of<any>({data:[]}) 
        }
  ...
  listar(pagina:number): void {
    ...
    this.cargando = true;
    this.principalService.lista_notificacion(pagina,this.resultadosPorPagina, 1).subscribe(
        resultado => {
          
          this.cargando = false;
          this.lista_notificacion = resultado.data as Seguimiento[];
  ...
  cargar_catalogos(tipo_acta:number): void {
    ...
    eliminar(id:string):void
    {
        ...
        {
            this.cargando = true;
            this.principalService.elimina_notificacion(id).subscribe(
            resultado => {
              this.cargando = false;
              this.listar(1);
    ...
    agregar_registro()
    {
      ...
      if(this.id_objeto== 0)
      {
        this.principalService.agregar_notificacion(this.registrosModule.value).subscribe(
          resultado => {
            this.listar(1);
            this.cargando = false;
            this.mensajeExito = new Mensaje(true);
        ...
        this.principalService.editar_notificacion(this.id_objeto, this.registrosModule.value).subscribe(
          resultado => {
            this.listar(1);
            this.cargando = false;
            this.mensajeExito = new Mensaje(true);
    ...
    cargar_registro(Objeto:any)
    {
      ...
      this.principalService.ver_notificacion(Objeto.id).subscribe(
          resultado => {
            this.showForm = true;
    ...
    listarBusqueda(term:string ,pagina:number): void {
      ...
      this.cargando = true;
      this.principalService.buscar_notificacion(term, pagina, this.resultadosPorPaginaBusqueda).subscribe(
          resultado => {
            this.cargando = false;
    ...
    filtro(id:number, value:string):void
    {
      switch (id) {
        ...
        break;
        case 2:
          this.busqueda.procedimiento = value;
        break;
      ...
      {
        this.busquedaActivada = true;
        this.principalService.buscar_notificacion(this.ultimoTerminoBuscado, this.busqueda, 1, this.resultadosPorPaginaBusqueda).subscribe(
            resultado => {
              this.cargando = false;
