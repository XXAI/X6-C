...
export class AvisoComponent implements OnInit {
  ...
  constructor( 
    ...
    ngOnInit() {
      ...
      this.registrosModule = this.fb.group({
        ...
        modificacion_datos: ['', [Validators.required]],
        fecha_baja: ['', [Validators.required]]
      });
  
      this.title.setTitle("Registro Aviso de Funcionamiento");
      ...
      var busquedaSubject = this.terminosBusqueda
        ...
        .switchMap((term:string)  =>  { 
          ...
          this.cargando = true;
          return busquedas_registradas ? this.principalService.buscar_aviso(term, this.busqueda, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda) : Observable.of<any>({data:[]}) 
        }
  ...
  listar(pagina:number): void {
    ...
    this.cargando = true;
    this.principalService.lista_aviso(pagina,this.resultadosPorPagina, 1).subscribe(
        resultado => {
          
          this.cargando = false;
          this.lista_aviso = resultado.data as Seguimiento[];
  ...
  cargar_catalogos(tipo_acta:number): void {
    ...
    eliminar(id:string):void
    {
        ...
        {
            this.cargando = true;
            this.principalService.elimina_aviso(id).subscribe(
            resultado => {
              this.cargando = false;
              this.listar(1);
    ...
    agregar_registro()
    {
      ...
      if(this.id_objeto== 0)
      {
        this.principalService.agregar_aviso(this.registrosModule.value).subscribe(
          resultado => {
            this.listar(1);
            this.cargando = false;
            this.mensajeExito = new Mensaje(true);
        ...
        this.principalService.editar_aviso(this.id_objeto, this.registrosModule.value).subscribe(
          resultado => {
            this.listar(1);
            this.cargando = false;
            this.mensajeExito = new Mensaje(true);
    ...
    cargar_registro(Objeto:any)
    {
      ...
      this.principalService.ver_aviso(Objeto.id).subscribe(
          resultado => {
            this.showForm = true;
    ...
    listarBusqueda(term:string ,pagina:number): void {
      ...
      this.cargando = true;
      this.principalService.buscar_aviso(term, pagina, this.resultadosPorPaginaBusqueda).subscribe(
          resultado => {
            this.cargando = false;
    ...
    filtro(id:number, value:string):void
    {
      ...
      {
        this.busquedaActivada = true;
        this.principalService.buscar_aviso(this.ultimoTerminoBuscado, this.busqueda, 1, this.resultadosPorPaginaBusqueda).subscribe(
            resultado => {
              this.cargando = false;
