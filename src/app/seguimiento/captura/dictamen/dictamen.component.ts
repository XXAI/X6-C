...
export class DictamenComponent implements OnInit {
  ...
  CatalogoResultadoLesp: string[] = ['DENTRO DE ESPECIFICACIONES', 'FUERA DE ESPECIFICACIONES'];
  CatalogoAnomalias: string[] = ['SIN ANOMALIAS', 'ANOMALIAS MENORES', 'ANOMALIAS MAYORES', 'ANOMALIAS CRITICAS'];
  ...
  constructor( 
    ...
    ngOnInit() {
      ...
      this.registrosModule = this.fb.group({
        ...
        no_acta: ['', [Validators.required]],
        id_area_operativa: ['', [Validators.required]],
        id_dictaminador: ['', [Validators.required]],
        fecha_recepcion: ['', [Validators.required]],
        id_giro: ['', [Validators.required]],
        establecimiento: ['', [Validators.required]],
        id_tipo_dictamen: ['', [Validators.required]],
        fecha_dictamen: ['', [Validators.required]],
        id_resultado_lesp: ['', [Validators.required]],
        
        id_anomalias: ['', [Validators.required]],
        id_medida_seguridad: ['', [Validators.required]],
        
        no_notificacion: ['', [Validators.required]],
        fecha_emision: ['', [Validators.required]],
        fecha_entrega_notificar: ['', [Validators.required]],
        fecha_notificacion: ['', [Validators.required]],
        id_respuesta_usuario: ['', [Validators.required]],
        acta_seguimiento_numero: ['', [Validators.required]],
        id_estatus: ['', [Validators.required]],
        fecha_envio_resolucion: ['', [Validators.required]],
      });
  
      this.title.setTitle("Registro Dictamen Técnico");
      this.mensajeError = new Mensaje();
      this.mensajeExito = new Mensaje();
  
      this.listar(1);
      this.cargar_catalogos(2);
      ...
      var busquedaSubject = this.terminosBusqueda
        ...
        .switchMap((term:string)  =>  { 
          ...
          this.cargando = true;
          return busquedas_registradas ? this.principalService.buscar_dictamen(term, this.busqueda, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda) : Observable.of<any>({data:[]}) 
        }
  ...
  listar(pagina:number): void {
    ...
    this.cargando = true;
    this.principalService.lista_dictamen(pagina,this.resultadosPorPagina, 1).subscribe(
        resultado => {
          
          this.cargando = false;
          this.lista_dictamen = resultado.data as Seguimiento[];
  ...
  cargar_catalogos(tipo_acta:number): void {
    ...
    eliminar(id:string):void
    {
        ...
        {
            this.cargando = true;
            this.principalService.elimina_dictamen(id).subscribe(
            resultado => {
              this.cargando = false;
              this.listar(1);
    ...
    agregar_registro()
    {
      ...
      if(this.id_objeto== 0)
      {
        this.principalService.agregar_dictamen(this.registrosModule.value).subscribe(
          resultado => {
            this.listar(1);
            this.cargando = false;
            this.mensajeExito = new Mensaje(true);
        ...
        this.principalService.editar_dictamen(this.id_objeto, this.registrosModule.value).subscribe(
          resultado => {
            this.listar(1);
            this.cargando = false;
            this.mensajeExito = new Mensaje(true);
    ...
    cargar_registro(Objeto:any)
    {
      ...
      this.principalService.ver_dictamen(Objeto.id).subscribe(
          resultado => {
            this.showForm = true;
    ...
    listarBusqueda(term:string ,pagina:number): void {
      ...
      this.cargando = true;
      this.principalService.buscar_dictamen(term, pagina, this.resultadosPorPaginaBusqueda).subscribe(
          resultado => {
            this.cargando = false;
    ...
    filtro(id:number, value:string):void
    {
      ...
      {
        this.busquedaActivada = true;
        this.principalService.buscar_dictamen(this.ultimoTerminoBuscado, this.busqueda, 1, this.resultadosPorPaginaBusqueda).subscribe(
            resultado => {
              this.cargando = false;
