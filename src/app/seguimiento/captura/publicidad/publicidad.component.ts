...
export class PublicidadComponent implements OnInit {
  ...
  busqueda: any = {buscarText:'', resolucion:'0'};
  lista_publicidad: Seguimiento[] = [];
  ...
  //catalogos
  CatalogoGiro: string[];
  CatalogoMedioUtilizado: string[] = ['IMPRESO', 'RADIO', 'TELEVISIÓN', 'REDES SOCIALES', 'INTERNET'];
  CatalogoDictamen: string[] = ['SIN ANOMALIAS', 'CON ANOMALIAS'];
  CatalogoResolucionAdministrativa: string[] = ['CON SANCIÓN ADMINISTRATIVA', 'CONCLUIDO', 'POR ACUERDO'];
  CatalogoMulta: string[] = ['AMONESTACIÓN', 'MULTA', 'CLAUSURA', 'ARRESTO'];
  CatalogoMedidaSeguridad: string[] = ['SUSPENSIÓN DE SERVICIOS', 'PROHIBICION DE ACTOS DE USO'];
  ...
  constructor( 
    ...
    ngOnInit() {
      ...
      this.registrosModule = this.fb.group({
        ...
        no_informe_verificacion: ['', [Validators.required]],
        id_medio_utilizado: ['', [Validators.required]],
        usuario_responsable: ['', [Validators.required]],
        id_dictamen: ['', [Validators.required]],
        id_medida_seguridad: ['', [Validators.required]],
        id_resolucion_administrativa: ['', [Validators.required]],
        id_sancion_administrativa: ['', [Validators.required]],
        fecha_emision: ['', [Validators.required]]
      });
  
      this.title.setTitle("Registro Publicidad");
      ...
      var busquedaSubject = this.terminosBusqueda
        ...
        .switchMap((term:string)  =>  { 
          ...
          if(this.busqueda.buscarText.length == 0 && this.busqueda.resolucion == 0)
          {
          ...
          this.cargando = true;
          return busquedas_registradas ? this.principalService.buscar_publicidad(term, this.busqueda, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda) : Observable.of<any>({data:[]}) 
        }
  ...
  listar(pagina:number): void {
    ...
    this.cargando = true;
    this.principalService.lista_publicidad(pagina,this.resultadosPorPagina, 1).subscribe(
        resultado => {
          
          this.cargando = false;
          this.lista_publicidad = resultado.data as Seguimiento[];
  ...
  cargar_catalogos(tipo_acta:number): void {
    ...
    eliminar(id:string):void
    {
        ...
        {
            this.cargando = true;
            this.principalService.elimina_publicidad(id).subscribe(
            resultado => {
              this.cargando = false;
              this.listar(1);
    ...
    agregar_registro()
    {
      ...
      if(this.id_objeto== 0)
      {
        this.principalService.agregar_publicidad(this.registrosModule.value).subscribe(
          resultado => {
            this.listar(1);
            this.cargando = false;
            this.mensajeExito = new Mensaje(true);
        ...
        this.principalService.editar_publicidad(this.id_objeto, this.registrosModule.value).subscribe(
          resultado => {
            this.listar(1);
            this.cargando = false;
            this.mensajeExito = new Mensaje(true);
    ...
    cargar_registro(Objeto:any)
    {
      ...
      this.principalService.ver_publicidad(Objeto.id).subscribe(
          resultado => {
            this.showForm = true;
    ...
    listarBusqueda(term:string ,pagina:number): void {
      ...
      this.cargando = true;
      this.principalService.buscar_publicidad(term, pagina, this.resultadosPorPaginaBusqueda).subscribe(
          resultado => {
            this.cargando = false;
    ...
    filtro(id:number, value:string):void
    {
      switch (id) {
        ...
        break;
        case 2:
          this.busqueda.resolucion = value;
        break;
      ...
      {
        this.busquedaActivada = true;
        this.principalService.buscar_publicidad(this.ultimoTerminoBuscado, this.busqueda, 1, this.resultadosPorPaginaBusqueda).subscribe(
            resultado => {
              this.cargando = false;
